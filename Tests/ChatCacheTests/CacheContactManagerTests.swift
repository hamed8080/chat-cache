...
final class CacheContactManagerTests: XCTestCase, CacheLogDelegate {
    ...
    func test_whenGetContacts_withUniqueIdReturnAndContact() {
        // Given
        let entity = CDContact.insertEntity(sut.viewContext)
        entity.uniqueId = "TEST_UNIQUEID"
        entity.id = 1
        entity.userId = 1

        let autoGeneratedEntityUnqiueId = CDContact.insertEntity(sut.viewContext)
        autoGeneratedEntityUnqiueId.id = 2
        autoGeneratedEntityUnqiueId.userId = 2
        sut.viewContext.perform {
            try? self.sut.viewContext.save()
        }

        // When
        let exp = expectation(description: "Expected to get a contact with a uniqueId.")
        notification.onInsert { (entities: [CDContact]) in
            self.sut.getContacts(.init(uniqueId: "TEST_UNIQUEID")) { entities, total in
                if entities.first?.id == 1, (entities.first(where: {$0.uniqueId == "TEST_UNIQUEID" }) != nil)  {
                    exp.fulfill()
                }
            }
        }
